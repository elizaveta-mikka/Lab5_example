...
def create_file(row, col):  # Метод для записи в файл исходной матрицы
    a_2 = np.zeros((row, col))  # Создание двумерного массива
    file = open("Lab2.txt", "w")  # Открытие файла для записи
    file.write("Исходная прямоугольная матрица:\n")
    for i in range(row):
        for j in range(col):
            a_2[i][j] = random.randint(-100, 100)  # Рандомное заполнение исходной матрицы элементами
            file.write(str(a[i][j]) + " ")
        file.write("\n")
    file.close()  # Файл закрывается
    return a_2  # Сформированная матрица передаётся для дальнейшей работы с ней
